...
# 차량 및 건물 노드 접근
car_node = supervisor.getFromDef("TeslaModel3")  # 'car'는 정확한 DEF 이름이어야 함
building_node = supervisor.getFromDef("BUILDING")  # 'BUILDING'은 정확한 DEF 이름이어야 함
...
while driver.step() != -1:
    ...
    print("\nBuilding position: ",  building_node.getPosition())  # Building GPS
    print('-------- END --------')
